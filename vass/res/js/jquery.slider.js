...
(function($){
	var SLD = function(element){
	    ...
	    var opciones = {
			...
			imgControlPrev: 'obsi_carrou_prev.png',
			imgControlNext: 'obsi_carrou_next.png',
			imgControlPlay: 'obsi_carrou_play.png',
			imgControlPause: 'obsi_carrou_pause.png',
		...
		var ajustarAlto = function() {
			...
			if (parentSinAlto) {
				...
				if (parentAlto>0) {
					...
					$(el).parent().parent().parent().animate({height:altoAux+'px'}, opciones.duracion/2);
				}else{
		...
		this.create = function(){
			if (!created) {
				...
				switch(opciones.modo){
				case 'tabs':
					opciones.controles = Boolean(false);
					opciones.textos = Boolean(false);
					...
					$(el).parent().parent().append('<div class="slider_tabs fix_slider_tabs"><ul class="slider_tabs fix_slider_tabs" /></div>');
					$(el).find('> li').each(function(i){
